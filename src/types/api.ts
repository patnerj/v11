...
export interface AiCopilotChatResponse {
  ...
  model: string
  headroom: AiHeadroomResponse
  open_positions: number
  news_warnings: AiNewsWarning[]
  timestamp: string
}

export interface AiSafeLotResponse {
  symbol: string
  balance: number
  risk_pct: number
  cash_at_risk: number
  sl_pips: number
  safe_lots: number
  max_lots_margin: number
  recommended_action: string
}

export interface AiHeadroomResponse {
  initial_balance: number
  current_balance: number
  current_equity: number
  daily_limit_pct: number
  daily_headroom: number
  daily_headroom_pct: number
  max_limit_pct: number
  max_headroom: number
  max_headroom_pct: number
  risk_status: 'safe' | 'caution' | 'critical' | 'breached'
}

export interface AiNewsWarning {
  id: number
  title: string
  currency: string
...
export interface AiSettings {
  ...
  ai_sentinel_autoban: number
  service_key: string
  max_tokens: number
  temperature: number
}

export interface AiSentinelReport {
  status: string
  audit_timestamp: string
  threat_level: 'NOMINAL' | 'ELEVATED' | 'HIGH' | 'CRITICAL'
  threat_score: number
  syndicate_clusters: Array<{
    symbol: string
    account_1: number
    account_2: number
    lots_1: number
  ...
  toxic_scalpers: Array<{
    account_id: number
    user_id: number
    user_email: string
    display_name: string
    fast_trades_count: number
    avg_duration_sec: number
  }>
  at_risk_breaches: Array<{
    account_id: number
    user_id: number
    balance: number
    equity: number
...
export interface SystemDoctorStatus {
  version: string
  platform: string
...
export interface AiTradeAutopsy {
  id: number
  trade_id: number
  account_id: number
  symbol: string
...
export interface AiJournalHistoryResponse {
  autopsies: AiTradeAutopsy[]
  scorecard: AiPsychologyScorecard
}

export interface AiTiltStatus {
  account_id: number
  is_tilted: boolean
  consecutive_losses: number
  recent_loss_amount: number
  cooling_off_active: boolean
  cooling_off_until: string | null
  cooling_off_remaining_sec: number
  reason: string | null
  recommended_action: string
}
