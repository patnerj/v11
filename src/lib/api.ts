...
import type {
  ...
  AiTradeAutopsy, AiJournalHistoryResponse, AiTiltStatus, CoolingOffResult,
} from '@/types/api'
...
export const api = {
  ...
  tickets: {
    ...
    create: (body: { subject: string; category: string; message: string }) => fxsim<{ success: boolean; id: number; ai_resolved?: boolean; ai_reply?: string }>('/tickets/create', { body }),
    reply:  (id: number, message: string) => fxsim<{ success: boolean }>(`/tickets/${id}/reply`, { body: { message } })
  },

  // ── AI Services: Copilot, Journal & Analysis ─────────────────────────
  ai: {
    copilot: {
  ...
  admin: {
    ...
    demoGenerate:  () => fxsim<{ success: boolean; users: number; accounts: number; orders: number; payouts: number; banners: number; message?: string }>('/admin/demo/generate', { body: {}, timeout: 120_000 }),
    demoSeed:      () => fxsim<{ success: boolean; users?: number; accounts?: number; orders?: number; payouts?: number; banners?: number; message?: string }>('/admin/demo/seed', { body: {}, timeout: 120_000 }),
    demoRemove:    () => fxsim<{ success: boolean; removed?: Record<string, number>; message?: string }>('/admin/demo/remove', { body: {}, timeout: 120_000 }),
    demoPurge:     () => fxsim<{ success: boolean; message?: string }>('/admin/demo/purge', { body: {}, timeout: 120_000 }),
