...
const ADMIN_NAV: NavItem[] = [
  ...
  { href: '/admin/branding',   label: 'Branding',   icon: Palette, queryKey: ['admin', 'branding'] },
        ...
        {open && (
          <>
            ...
            <motion.aside
              ...
              className="lg:hidden fixed inset-y-0 left-0 w-[18rem] max-w-[85vw] flex-col bg-bg-subtle border-r border-border z-[70] flex shadow-2xl pt-[max(0.75rem,env(safe-area-inset-top,0px))] pb-[max(3rem,env(safe-area-inset-bottom,0px))]"
            >
              <div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle shrink-0">
                <Link 
                  href={role === 'admin' ? '/admin' : '/dashboard'} 
                  onClick={onClose} 
                  className="focus-ring rounded-md flex items-center gap-2 min-w-0 flex-1"
                >
              ...
              <SidebarBody role={role} currentPath={currentPath} isMobileDrawer={true} onCloseMobile={onClose} />
            </motion.aside>
          </>
        )}
      </AnimatePresence>
...
function SidebarBody({ 
  ...
  return (
    <>
      ...
      {onToggleCollapse && collapsed && !isMobileDrawer && (
      ...
      <div className={cn('border-t border-border-subtle space-y-2', collapsed ? 'px-2 py-4' : 'px-3 py-4 pb-4')}>
        {collapsed ? (
          <Link href="/faq" title="Help & docs" onClick={onCloseMobile}
          ...
          <>
            ...
            <Button asChild size="sm" variant="outline" className="w-full" onClick={onCloseMobile}>
