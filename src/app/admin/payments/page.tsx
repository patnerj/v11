...
export default function PaymentsHubPage() {
  ...
  useEffect(() => {
    ...
    const fetchModalProof = async () => {
      try {
        ...
        // P3 SECURITY: bearer travels in headers ONLY. Old code also appended
        // ?token= to the URL — the backend never reads it (it checks the
        // Authorization / X-FXSIM-Token headers), so it only leaked the token
        // into logs, history and referers.
        ...
        const res = await fetch(baseDocUrl, { credentials: 'include', headers, cache: 'no-store' })
  ...
  const viewProof = async (relativePath: string) => {
    try {
      ...
      // P3: headers-only auth (same as fetchModalProof above — no ?token= leak).
