...
function CheckoutInner() {
  ...
  // P3: money-action in-flight guard. The shared `loading` boolean flips back
  // and forth across coupon/gateway/proof steps, so a double-click could fire
  // paymentCreate twice and mint two pending orders (backend has no
  // idempotency key on /payment/create). This ref drops re-entrant calls.
  ...
  async function chooseGateway(g: Gateway) {
    if (!g || !plan) return
    // P3: drop re-entrant clicks while an order/checkout call is in flight.
