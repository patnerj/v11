...
/**
 * P3: single optimistic-ID scheme. Three call sites used to mint ghost ids
 * three different ways (-Date.now() vs -random()*1e6), risking collisions
 * with each other and with real ids. Negative Date.now() plus a per-process
 * counter: always negative (real ids are positive), unique even for two
 * same-millisecond orders.
 */
let optimisticSeq = 0
export function nextOptimisticId(): number {
  optimisticSeq = (optimisticSeq + 1) % 1000
  return -(Date.now() * 1000 + optimisticSeq)
}
