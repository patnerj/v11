...
interface AuthState {
  ...
  signinWithBiometricToken: (token: string) => Promise<{ ok: boolean; error?: string }>
...
export const useAuth = create<AuthState>((set, get) => ({
  ...
  signinWithBiometricToken: async (token: string) => {
    ...
    const res = await api.auth.me(true)
    if (res.ok && res.data) {
      set({ user: res.data, loading: false, ready: true, lastChecked: Date.now(), error: null })
      return { ok: true }
    }
  ...
  verifyTwoFactor: async (uid: number, code: string, remember?: boolean) => {
    ...
    if (res.ok) {
      ...
      try {
        const { saveBiometricSession } = await import('@/lib/biometrics')
        const u = (res.data.user as any)?.user_login || (res.data.user as any)?.username || ''
